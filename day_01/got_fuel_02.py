...
def added_fuel_required_for_module(module): # 14
    ...
    total += fuel
    while is_further_fuel_required(fuel):
        fuel = fuel_required_for_module(fuel)
        total += fuel
    return total

def is_further_fuel_required(initial_fuel):
    return fuel_required_for_module(initial_fuel) > 0

def compute_total_additional_fuel_required(modules):
    total = 0
    for each in modules:
